...
from repvit_cfgs import repvit_L_cfgs, repvit_m1_5_cfgs, repvit_m1_0_cfgs
...
parser.add_argument(
    ...
    type=str,
    default="workdir/finetune/finetune_weights.pth",
...
def MedSAM_infer_npz_3D(img_npz_file):
    ...
    for idx, box3D in enumerate(boxes_3D, start=1):
        segs_3d_temp = np.zeros_like(img_3D, dtype=np.uint8) # 3D slice seg
        x_min, y_min, z_min, x_max, y_max, z_max = box3D # bbox where rois locate
        ...
        for z in range(z_middle, z_max):
            ...
            if len(img_2d.shape) == 2:
                img_3c = np.repeat(img_2d[:, :, None], 3, axis=-1) # create a 2d slice with 3 repeated channels
            else:
                img_3c = img_2d
            H, W, _ = img_3c.shape # HxWx3
            ...
            with torch.no_grad():
                image_embedding = medsam_lite_model.image_encoder(img_256_tensor) # (1, 256, 64, 64) 172MB mem incr
            if z == z_middle:
                box_256 = resize_box_to_256(mid_slice_bbox_2d, original_size=(H, W)) # resize bbox
            else:
                pre_seg = segs[z-1, :, :]
                if np.max(pre_seg) > 0:
...
def validate(model, val_root):
    val_set = sorted(glob(join(val_root, '*.npz'), recursive=True))
    for img_npz_file in tqdm(val_set):
    start_time = time()
